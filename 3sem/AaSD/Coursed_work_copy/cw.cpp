...
std::map<char, Shannon_Fano> Shannon_Fano_Table(unsigned int CharWeight[256], std::string text){
...
void Shannon_Fano_encode(std::ifstream& weight_in, std::ifstream &file_in,std::ofstream &file_out){
    size_t weights_size;
    weight_in >> weights_size;
    unsigned int CharWeight[256];
    for(size_t i =0; i < 256; i++) CharWeight[i] = 0;
    ...
    std::map<char,Shannon_Fano> Table = Shannon_Fano_Table(CharWeight, buff);
...
int main(){
    std::ifstream weights_in("Weight");
    std::ifstream file_in("Code"); 
    std::ofstream file_out("Encode");
    Shannon_Fano_encode(weights_in, file_in,file_out);
    weights_in.close();
    file_in.close();
    file_out.close();
